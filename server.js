...
app.put('/tickets/:id', async (req, res) => {
	const id = parseInt(req.params.id)
	const { assignee, priority, status, description, ticket_id } = req.body

	pool.query(
		'UPDATE ticket_item assignee = $1, priority = $2, status = $3, description = $4 WHERE ticket_id = $5',
		[assignee, priority, status, description, ticket_id],
		(error, results) => {
			if (error) {
				throw error
			}
